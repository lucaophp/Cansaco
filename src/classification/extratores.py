...
lbpRaio = 2
nBins = 18
momentsOrder = [(0, 1), (1, 0), (1, 1), (0, 2), (2, 0)]
...
class Extratores(object):
    ...
    def momentos(self):
        ...
        nomes_m = [M + str(p) + str(q) for M, (p, q) in zip(['M_'] * len(momentsOrder), momentsOrder)]
        ...
        nomes_mu = [M + str(p) + str(q) for M, (p, q) in zip(['Mu_'] * len(momentsOrder), momentsOrder)]

        nu = measure.moments_normalized(mu)
        hu = measure.moments_hu(nu)

        valores_hu = list(hu)
        nomes_hu = [m + n for m, n in zip(['Hu_'] * len(valores_hu), map(str, range(0, len(valores_hu))))]
    ...
    def matriz_coocorrencia(self):
        ...
        g = feature.greycomatrix(self.imagemTonsDeCinza, [1, 2], [0, np.pi / 4, np.pi / 2], glcmNiveis, normed=True,
                                 symmetric=True)
        ...
        def compute_feats(image, kernels):
            ...
            for k, kernel in enumerate(kernels):  # enumerate-> Retorna uma tupla contendo uma contagem
                filtered = ndi.convolve(image, kernel,
                                        mode='wrap')  # ndi.convolve-> Retorna o resul da convolução de entrada com pesos
